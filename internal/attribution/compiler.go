...
var releaseApplicationIDPattern = regexp.MustCompile(`^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-8][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$`)
...
func Init(root string) (InitResult, error) {
	...
	lines = append(lines,
		...
		"    associatedDomains:",
		"      - attribution.sh",
		"    # Source/publisher apps only: set true before adding ad-network identifiers.",
		"    publisherMode: false",
		"    skAdNetworkIds: []",
	)
	if project.Host == "expo" {
...
func buildExpoPlan(project Project) (Plan, error) {
	root := project.Root
	for _, path := range []string{"app.json", ConfigPath, ".attribution/.gitignore", PluginPath, ExpoFacadePath, ManifestPath} {
	...
	facade := renderExpoFacade(config)
	...
	releaseManifest := releaseManifestForProject(root, config, configDigest, schemaDigest)
	wrapper, err := renderWrapper(config, schemaDigest, releaseManifest)
	if err != nil {
		return Plan{}, err
	}
	...
	manifest := GeneratedManifest{
		...
		GeneratedFiles: []GeneratedFile{
			{Path: PluginPath, SHA256: sha256Hex(wrapper)},
			{Path: ExpoFacadePath, SHA256: sha256Hex(facade)},
		},
	}
	...
	operations := []Operation{
		...
		{Path: ExpoFacadePath, Content: facade},
...
func buildSwiftUIPlan(project Project) (Plan, error) {
	...
	releaseManifest := releaseManifestForProject(project.Root, config, configDigest, schemaDigest)
	plist, err := renderSwiftPlist(config, schemaDigest, releaseManifest)
	if err != nil {
		return Plan{}, err
	}
...
func renderSwiftSource(config Config, schemaDigest string) []byte {
	...
	for index, event := range config.Schema.Events {
		...
		eventCases = append(eventCases, "    case "+event+" = "+quoteSwift(event))
	}
	source := `// Generated by attribution — do not edit. Regenerate with ` + "`attribution apply`" + `.
import AttributionCore
import Foundation

enum AttributionKitEvent: String, Sendable {
` + strings.Join(eventCases, "\n") + `
}
...
` + strings.Join(eventPairs, ",\n") + `
    ...
    static func ready() async throws {
        try await Attribution.ready()
    }

    static func track(_ event: AttributionKitEvent) async throws {
        try await Attribution.track(event.rawValue)
    }
...
func renderExpoFacade(config Config) []byte {
	quoted := make([]string, 0, len(config.Schema.Events))
	propertyRows := make([]string, 0, len(config.Schema.Events))
	for _, event := range config.Schema.Events {
		quoted = append(quoted, quoteYAML(event))
		propertyRows = append(propertyRows, "  "+event+": Record<string, never>;")
	}
	return []byte(`// Generated by attribution — do not edit. Regenerate with ` + "`attribution apply`" + `.
import { Attribution, type AttributionConsentState } from "@attributionkit/expo";

export const attributionEvents = [` + strings.Join(quoted, ", ") + `] as const;
export type AttributionEvent = (typeof attributionEvents)[number];
export interface AttributionEventProperties {
` + strings.Join(propertyRows, "\n") + `
}
...
func renderSwiftPlist(config Config, schemaDigest string, releaseManifest json.RawMessage) ([]byte, error) {
...
<dict>
  ...
  <string>`)
	...
	body.WriteString(escapeXML(endpoint))
	body.WriteString("</string>\n")
	if config.Providers.Apple.PublisherMode {
		body.WriteString("  <key>SKAdNetworkItems</key>\n  <array>\n")
	...
	if len(releaseManifest) > 0 {
		body.WriteString("  <key>AttributionKitReleaseManifestJSON</key>\n  <string>")
		body.WriteString(escapeXML(string(releaseManifest)))
		body.WriteString("</string>\n")
	}
	body.WriteString(`  <key>AttributionKitSchemaHash</key>
  <string>`)
...
func renderWrapper(config Config, schemaDigest string, releaseManifest json.RawMessage) ([]byte, error) {
	...
	type options struct {
		...
		PublisherMode                  bool     `json:"publisherMode"`
		...
		ReleaseManifest                any      `json:"releaseManifest,omitempty"`
	}
	values := options{
		Endpoint:                       endpoint.String(),
		AssociatedDomains:              config.Providers.Apple.AssociatedDomains,
		PublisherMode:                  config.Providers.Apple.PublisherMode,
	...
	if len(releaseManifest) > 0 {
		var decoded any
		if err := json.Unmarshal(releaseManifest, &decoded); err != nil {
			return nil, err
		}
		values.ReleaseManifest = decoded
	}
	if config.Providers.Meta != nil {
...
func releaseManifestForProject(root string, config Config, configDigest, schemaDigest string) json.RawMessage {
	binding, err := ReadCloudBinding(root)
	if err != nil || binding.BundleID != config.App.BundleID {
		return nil
	}
	if !releaseApplicationIDPattern.MatchString(binding.ApplicationID) {
		return nil
	}
	endpoint, err := normalizedEndpoint(config.Providers.Apple.Endpoint)
	if err != nil {
	...
	events := make([]map[string]any, 0, len(config.Schema.Events))
	for value, name := range config.Schema.Events {
		events = append(events, map[string]any{
			"name": name, "properties": map[string]any{}, "fineConversionValue": value,
			"lockPostback": false,
		})
	}
	authority := "managed_apple"
	if config.Mode == "external" {
	...
	manifest := map[string]any{
		...
		"collectorOrigin":         endpoint,
		"conversionAuthority":     authority,
		"conversionSchemaVersion": "apple_conversion_1",
		"eventSchemaVersion":      "events_4",
		"enabledAppleDrivers":     []string{"aak", "skan"},
		"associatedDomains":       config.Providers.Apple.AssociatedDomains,
		"configurationDigest":     "sha256:" + configDigest,
		"events":                  events,
		"appAttestEnabled":        false,
		"eventSchemaDigest":       "sha256:" + schemaDigest,
	}
